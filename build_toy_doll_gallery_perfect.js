...
const galleryDir = 'd:/validation/museum/ToyAndDollGallery';
...
const itemCount = galleryItems.hindi ? galleryItems.hindi.length : 9;
...
function generatePage(itemIndex, relPath) {
  ...
  languages.forEach((langObj) => {
    ...
    const items = galleryItems[lang] || galleryItems.hindi || [];
    ...
    const imgPath = item.image ? `${relPath}${item.image}` : '';
    const audioPath = item.audioSrc ? `${relPath}${encodeURI(item.audioSrc)}` : '';
    audioMap[lang] = audioPath;

    const title = item.title || `Item ${itemNum}`;
    const badge = item.badge || '';
    const desc = item.desc ? item.desc.split('\n\n').join('<br><br>') : '';

    langDivsHtml += `
    <div class="bookContentDiv langCnt" id="${lang}" style="display: ${isDisplay};">
      <h2 class="cntHdng" ${isRtl}>${title}</h2>
      <div class="booImgDiv">
        <img src="${imgPath}" width="100%" alt="${title}" onerror="this.onerror=null; this.src='${relPath}images/toy-and-doll-item${itemNum}.png';">
      </div>
...
<head>
  ...
  <title>Toy & Doll Gallery - Item ${itemNum} | Salar Jung Museum</title>
...
for (let i = 0; i < itemCount; i++) {
  ...
  console.log(`Generated item${itemNum}.html and Item${itemNum}/index.html for ToyAndDollGallery`);
}

...
const galleryDir = 'd:/validation/museum/UtilityWareGallery';
const appJsPath = path.join(galleryDir, 'js/app.js');

const appJsStr = fs.readFileSync(appJsPath, 'utf8');

// Extract itemsData and itemAudioByLanguage using Function/eval
const matchAudioData = appJsStr.match(/const itemAudioByLanguage = (\{[\s\S]*?\n  \};\n)/);
const matchItemsData = appJsStr.match(/const itemsData = (\{[\s\S]*?\n  \};\n)/);

let itemAudioByLanguage = {};
let itemsData = {};

if (matchAudioData) {
  eval('itemAudioByLanguage = ' + matchAudioData[1].replace(/;\s*$/, ''));
}
if (matchItemsData) {
  eval('itemsData = ' + matchItemsData[1].replace(/;\s*$/, ''));
}
...
function getAudioPath(lang, itemIndex) {
  const audioInfo = itemAudioByLanguage[lang] || itemAudioByLanguage.hindi;
  if (!audioInfo || !audioInfo.folder || !audioInfo.files) return '';
  const folder = audioInfo.folder;
  const fileName = audioInfo.files[itemIndex];
  if (!fileName) return '';
  
  const fullPathOnDisk = path.join(galleryDir, 'audio', folder, fileName);
...
function generatePage(itemIndex, relPath) {
  ...
  const imgPath = `${relPath}images/utility-item${itemNum}.jpg`;
  const audioMap = {};
  let langDivsHtml = '';
  ...
  languages.forEach((langObj) => {
    ...
    <div class="bookContentDiv langCnt" id="${lang}" style="display: ${isDisplay};">
      ...
      <div class="booImgDiv">
        <img src="${imgPath}" width="100%" alt="${escapeAttr(title)}" onerror="this.onerror=null; this.src='${relPath}images/utility-item${itemNum}.jpg';">
      </div>
...
<head>
  ...
  <title>Utility Ware Gallery - Item ${itemNum} | Salar Jung Museum</title>
...
for (let i = 0; i < itemCount; i++) {
  ...
  console.log(`Generated item${itemNum}.html and Item${itemNum}/index.html for UtilityWareGallery`);
}

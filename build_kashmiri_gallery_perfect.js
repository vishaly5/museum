...
function generatePage(itemIndex, relPath) {
  ...
  languages.forEach((langObj) => {
    ...
    const imgPath = item.image ? `${relPath}${item.image}` : `${relPath}images/kashmiri-item${itemNum}.jpg`;
    ...
    <div class="bookContentDiv langCnt" id="${lang}" style="display: ${isDisplay};">
      ...
      <div class="booImgDiv">
        <img src="${imgPath}" width="100%" alt="${escapeAttr(title)}" onerror="this.onerror=null; this.src='${relPath}images/kashmiri-item${itemNum}.jpg';">
      </div>
...
<head>
  ...
  <title>Kashmiri Gallery - Item ${itemNum} | Salar Jung Museum</title>
...
for (let i = 0; i < itemCount; i++) {
  ...
  console.log(`Generated item${itemNum}.html and Item${itemNum}/index.html for KashmiriGallery`);
}
